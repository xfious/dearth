...
from distill import destory, distill_init, distill_run, set_get_batch_func, save_student, get_current_iter
from dearth_model import DearthLM, DearthConfig
from distill_util import Distill_train_config
from myMistral import MistralForCausalLM
...
def setup_args():
    ...
    parser.add_argument('-s', '--start_iter', type=int, default=None, help='start iteration')
    parser.add_argument('-i', '--iters', type=int, default=None, help='run i more iterations')
    parser.add_argument('-nt', '--n_teacher', type=int, default=1, help='number of teacher')
    parser.add_argument('-ns', '--n_student', type=int, default=1, help='number of student')
...
def get_model_config(args, yaml_config, teacher_model, tokenizer) -> DearthConfig:
    ...
    if ret.hidden_dim is None:
        ret.hidden_dim = ret.dim * 4
    print(ret)
    return ret
...
def set_critical_env(args, yaml_config):
    ...
    if _training_config.ckpt_path is not None:
        ...
        _start_iter = states["iter"]
        logging.info(f"start_iter: {_start_iter} from ckpt {_training_config.ckpt_path}")

    if 'start_iter' in yaml_config and not args.rnew:
        _start_iter = yaml_config['start_iter']
        logging.info(f"start_iter: {_start_iter} from config")
    if args.start_iter is not None:
        _start_iter = args.start_iter
        logging.info(f"start_iter: {_start_iter} from args")
    if _start_iter < 0:
        _start_iter = 0
    if 'n_more_iter' in yaml_config:
        _n_more_iter = yaml_config['n_more_iter']
    if args.iters is not None:
        _n_more_iter = args.iters
    logging.info(f"n_more_iter: {_n_more_iter}")
...
def main(args, yaml_config):
    # prepare tokenizer
    teacher_model_name = "HuggingFaceH4/zephyr-7b-alpha"
    ...
    model_config.update({"target_layer_idx": 28})
    teacher = MistralForCausalLM.from_pretrained(
    ...
    max_epoch = _n_more_iter // _epoch_size
    for i in range(max_epoch):
        distill_run(batch_size=_batch_size, large_batch_size=_large_batch_size, iter_cnt=_epoch_size)
        save_ckpt(f"{_training_name}-{int(get_current_iter())}.pt")
    time.sleep(1)
    destory()
    dl.destory()
