...
class GPTNeoSelfAttention(nn.Module):
    ...
    def forward(
        ...
        saved_v = distill_util._post_process_v(value) # TODO: modified
        ...
        if output_attentions: # TODO: modified
        ...
        if output_attentions:
            outputs += ((distill_util.post_process_attn(attn_weights), saved_v),) # TODO: modified
...
class GPTNeoModel(GPTNeoPreTrainedModel):
    def __init__(self, config):
        ...
        # TODO: added
        # self.target_layer_idx = config.target_layer_idx if hasattr(config, "target_layer_idx") else int(config.num_hidden_layers * 0.85)
        if hasattr(config, "target_layer_idx"):
            self.target_layer_idx = config.target_layer_idx
        else:
            logger.warning(f"No target layer index is provided, using the default value 0.85 * num_hidden_layers, which is {int(config.num_hidden_layers * 0.85)}")
            self.target_layer_idx = int(config.num_hidden_layers * 0.85)
        print(f"Target layer index: {self.target_layer_idx}")
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                outputs = block(
                    ...
                    use_cache=use_cache,
                    output_attentions=output_attentions and i == self.target_layer_idx, # TODO: modified
                )
            ...
            if output_attentions and i == self.target_layer_idx: # TODO: modified
...
class GPTNeoForCausalLM(GPTNeoPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            shift_logits = lm_logits[..., :-1, :].contiguous() # current shape: (batch_size, seq_len, vocab_size)
            shift_labels = labels[..., 1:].contiguous()
            # Flatten the tokens
            loss_fct = CrossEntropyLoss()
            # shape of shift_logits.view(-1, shift_logits.size(-1)): (batch_size * seq_len, vocab_size)
            # shape of shift_labels.view(-1): (batch_size * seq_len)
        ...
        if not return_dict:
            output = (distill_util._post_process_logits(lm_logits),) + transformer_outputs[1:] # TODO: modified
            return ((loss,) + output) if loss is not None else output

        return CausalLMOutputWithPast(
            loss=loss,
            logits=distill_util._post_process_logits(lm_logits), # TODO: modified
